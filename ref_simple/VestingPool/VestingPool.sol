...
import "./interfaces/IdbgPool.sol";
...
contract VestingPool is IVestingPool, Modifiers {
    ...
    constructor(
        ...
        m_vestingAmount = vestingMonths > 0 ? m_totalAmount / vestingMonths : 0;
    }
    ...
    function claim(uint poolId) external override onlyOwners(m_claimers) {
        ...
        m_recipient.transfer(unlocked, false, 2);
    ...
    function calcUnlocked() private view returns (uint128, uint32) {
        ...
        uint32 _now = m_dbgUnlockAll ? m_vestingEnd + 1 : uint32(now);
        if (_now > m_cliffEnd) {
            ...
            if (_now > m_vestingEnd) {
                ...
                unlocked = m_remainingAmount - 
                    math.min(m_remainingAmount, (vestingPeriods * m_vestingAmount));
    ...
    // Debug only
    //
    function unlock() public senderIs(creator) {
        m_dbgUnlockAll = true;
    }
}